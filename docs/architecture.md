...
The temporary development teacher-account action reauthorizes ADMIN, validates exact name/email/password fields, and uses the isolated privileged Auth client only for confirmed user creation and exact-new-user compensation. The normal authenticated client performs TEACHER profile provisioning through its hardened RPC; no password reaches the database layer or action response. This boundary is explicitly scheduled for removal before pilot onboarding.
...
The authenticated mobile drawer is a second narrow Client Component inside that Server Component shell. It is keyed by the full browser pathname so preserved App Router layouts cannot retain an open drawer across destination, locale, back, or forward navigation. The drawer owns only disclosure, focus, backdrop, inert-background, and body-scroll state; authorization and navigation-item selection remain server-owned.
