...
- Multi-row student workflows execute inside four hardened ADMIN RPCs. They use `auth.uid()`, schema-qualified empty-search-path code and deterministic row locking; they do not accept tenant/role authority, use the privileged Auth client, expose record existence across schools, or widen direct grants.
- Teacher assignment mutations use two hardened ADMIN RPCs. Capacity checks serialize on the parent ClassCourse row, authorization uses database profiles rather than JWT metadata, teacher access is SELECT-only and assignment-scoped, and inactive assignments revoke access without deletion.
...
- The temporary development-only teacher account form additionally permits that isolated client to create one confirmed Auth user from an ADMIN-submitted temporary password. The password is never logged, persisted, rendered back, or passed to the profile RPC; failed profile provisioning compensates only the newly created Auth identity. Remove this path before pilot use.
